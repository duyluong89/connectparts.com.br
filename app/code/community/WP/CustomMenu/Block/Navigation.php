...
class WP_CustomMenu_Block_Navigation extends Mage_Catalog_Block_Navigation
{
    ...
    public function drawCustomMenuItem($category, $level = 0, $last = false)
    {
        ...
        $htmlTop[] = '<style>
           .parentMenu a:hover .menu-icon.'.$cat->getData('class_category').'{
                background-position: '.$cat->getData("position_background_icon_hover").' !important;

            }
            .menu.active .menu-icon.'.$cat->getData('class_category').'{
                background-position: '.$cat->getData("position_background_icon_hover").' !important;
        ...
        $width_of_parent = $cat->getData("width_of_parent");
        $set_width = ($width_of_parent) ? "width: " . $width_of_parent :"";
        $htmlTop[] = '<div class="parentMenu">';
        if ($level == 0 && $drawPopup) {
            $htmlTop[] = '<a href="javascript:void(0);" style="min-height:69px;" rel="'.$this->getCategoryUrl($category).'">';
        } else {
        ...
        $position_top_menu = $cat->getData("position_background_icon");
        if($position_top_menu){
            $htmlTop[] = '<span class="menu-icon '.$cat->getData("class_category").'" style="background-position:'.$position_top_menu.'"></span>';
            
        }
        $htmlTop[] = '<span style="'.$set_width.'">' . $name . '</span>';
        $htmlTop[] = '</a>';
        $htmlTop[] = '</div>';
        $htmlTop[] = '</div>';
        $htmlTop[] = '<span class="line"></span>';
